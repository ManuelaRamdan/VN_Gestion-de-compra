...
export default function GestionEntrega({ compra, onBack, puedeEditar = true }) {
    ...
    const [reclamoActivo, setReclamoActivo] = useState(true); // NUEVO: Estado para saber si está inactivo
    ...
    const cargarTodo = async () => {
        try {
            ...
            try {
                // Buscamos el reclamo. Si existe (activo o inactivo), lo cargamos.
                const r = await getReclamoPorEvaluacion(evalData.idEvaluacionEntrega);
                setReclamoId(r.data.idReclamo);
                setReclamoActivo(r.data.activo !== false); // Asumimos true si no viene, false si es explicitamente false
                setReclamoForm({
    ...
    const handleGuardarEvaluacion = async (e) => {
        e.preventDefault();
        setModalError("");
        if (isEvaluacionLocked || !puedeEditar) return; 

        try {
            const dataToSend = {
                idCompra: compra.idCompra,
                fechaEntrega: formData.fechaEntrega,
                cumpleCondiciones: formData.cumpleCondiciones, 
            ...
            setModalError(error.response?.data?.error || error.message || "Error al guardar evaluación");
        }
    };
    ...
    const handleGuardarReclamo = async (e) => {
        ...
        try {
            ...
            setReclamoError(error.response?.data?.error || error.message || "Error al guardar reclamo");
        }
    };
        ...
        <div className="pb-10 animate-in fade-in duration-500">
            ...
            {isGlobalLocked && (
                <div className="mb-6 bg-gray-50 text-gray-700 p-3 rounded-lg text-sm flex items-center gap-2 border border-gray-200">
                    <Lock size={16} />
                    <span><strong>Proceso Finalizado:</strong> Este caso ha sido cerrado administrativamente.</span>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                ...
                <div>
                    ...
                    {evaluacion ? (
                            ...
                            {puedeEditar && (
                                <button
                                    ...
                                    className={`absolute top-3 right-3 transition-colors ${
                                        isEvaluacionLocked ? 'text-slate-300 cursor-not-allowed' : 'text-slate-400 hover:text-blue-600'
                                    }`}
                                >
                        ...
                        puedeEditar ? (
                            ...
                            <div className="w-full bg-slate-50 border border-slate-200 rounded-xl p-8 flex flex-col items-center justify-center text-center text-slate-500 gap-3">
                                <AlertCircle size={32} className="text-slate-300" />
                                <div className="font-medium text-base">Sin Evaluación</div>
                                <p className="text-xs text-slate-400">Esta entrega todavía no cuenta con una evaluación registrada.</p>
                            </div>
                        )
                    )}
                </div>
                ...
                {evaluacion && (
                    <div>
                        ...
                        {/* 1. SI SE ESTÁ EDITANDO/CREANDO EL RECLAMO */}
                        {mostrarReclamo ? (
                            ...
                            <div className={`bg-white border border-red-100 rounded-xl p-5 shadow-sm relative animate-in fade-in ${isReclamoLocked ? 'opacity-80' : ''}`}>
                                {puedeEditar && (
                                    <button
                                        ...
                                        className={`absolute top-3 right-3 transition-colors ${
                                            isReclamoLocked ? 'text-slate-300 cursor-not-allowed' : 'text-slate-400 hover:text-blue-600'
                                        }`}
                                    >
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center gap-2 text-slate-700">
                                        ...
                                        <span className="text-sm text-gray-500 ml-auto mr-6">{formatearFecha(reclamoForm.fechaReclamo)}</span>
                                    </div>
                                    <div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        ...
                                        {reclamoForm.entregaNueva && <span className="px-2 py-1 bg-blue-100 text-blue-700 text-xs rounded-full font-medium">Nueva Entrega</span>}
                                    </div>
                                    ...
                                    {reclamoForm.entregaNueva && (
                                        <div className="flex justify-between items-center mt-1">
                                            ...
                                            <span className={`text-xs font-bold ${reclamoForm.satisfechoConNuevaEntrega ? 'text-emerald-600' : 'text-red-600'}`}>
                                                {reclamoForm.satisfechoConNuevaEntrega ? 'Satisfecho' : 'Aún No Satisfecho'}
                                            </span>
                                        </div>
                                    )}
                                </div>
                            ...
                            <div className="w-full bg-slate-50 border border-slate-200 rounded-xl p-6 flex flex-col items-center justify-center text-center text-slate-500 gap-2">
                ...
                <div className="fixed inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm z-50 animate-in fade-in">
                    <div className="bg-white w-full max-w-md rounded-xl shadow-2xl animate-in zoom-in-95">
                        <div className="px-6 py-4 flex justify-between items-center border-b border-slate-100 bg-[#1C5B5A] text-white rounded-t-xl">
                            <h3 className="font-bold text-lg">{editingId ? "Modificar Evaluación" : "Registrar Evaluación"}</h3>
