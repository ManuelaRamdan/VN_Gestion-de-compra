...
export default function TablaPrioridad({
    prioridades,
    onEdit,
    onDeactivate,
    onLoadMore,
    ...
    // Filtro por categoría (nombre de la prioridad)
    const filteredData = prioridades.filter((p) => {
    ...
    return (
        <div className="bg-white rounded-lg shadow-sm border border-slate-100 overflow-hidden">
            <div className="p-6 border-b border-slate-50 flex flex-col sm:flex-row justify-between items-center gap-4">
                <h2 className="text-base font-bold text-slate-800">Niveles de Prioridad Registrados</h2>
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-50">
                        {filteredData.map((p) => (
                            <tr key={p.idNivelPrioridad} className="hover:bg-slate-50 transition-colors group">
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-slate-800">
                                    #{p.idNivelPrioridad}
                                </td>
                                <td className="px-6 py-4">
                                    <div className="font-bold text-slate-800 text-sm">{p.categoria}</div>
                                </td>
                                <td className="px-6 py-4">
                ...
                {filteredData.length === 0 && (
                    <div className="p-8 text-center text-gray-500 text-sm">
                        No se encontraron prioridades que coincidan con la búsqueda.
                    </div>
                )}
