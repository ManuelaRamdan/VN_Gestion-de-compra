...
export default function TablaGestionProveedor({
    proveedores,
    onEdit,
    onDeactivate,
    onLoadMore,
    ...
    // Filtro múltiple (Empresa, Contacto o Mail)
    ...
    return (
        <div className="bg-white rounded-lg shadow-sm border border-slate-100 overflow-hidden">
            <div className="p-6 border-b border-slate-50 flex flex-col sm:flex-row justify-between items-center gap-4">
                <h2 className="text-base font-bold text-slate-800">Proveedores Registrados</h2>
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-50">
                        {filteredData.map((p) => (
                            <tr key={p.idProveedor} className="hover:bg-slate-50 transition-colors group">
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-slate-800">
                                    #{p.idProveedor}
                                </td>
                                <td className="px-6 py-4">
                                    <div className="font-bold text-slate-800 text-sm">{p.nombreEmpresa}</div>
                                    <div className="text-xs text-gray-500">{p.nombreContacto || 'Sin contacto definido'}</div>
                                </td>
                                <td className="px-6 py-4">
                                    <div className="text-sm text-slate-700">{p.mail || '-'}</div>
