...
export default function TablaUsuario({
    ...
    // Filtro por username o sector
    const filteredData = usuarios.filter((u) => {
        const term = searchTerm.toLowerCase();
        return (
            u.username?.toLowerCase().includes(term) ||
            u.sector?.nombre?.toLowerCase().includes(term)
        );
    });
    ...
    return (
        <div className="bg-white rounded-lg shadow-sm border border-slate-100 overflow-hidden">
            <div className="p-6 border-b border-slate-50 flex flex-col sm:flex-row justify-between items-center gap-4">
                <h2 className="text-base font-bold text-slate-800">Usuarios Registrados</h2>

                <div className="relative w-full sm:w-64">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={16} />
                    <input
                        type="text"
                        placeholder="Buscar por usuario o sector..."
            ...
            <div className="overflow-x-auto">
                ...
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-50">
                        {filteredData.map((u) => (
                            <tr key={u.idUsuario} className="hover:bg-slate-50 transition-colors group">
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-slate-800">
                                    #{u.idUsuario}
                                </td>
                                <td className="px-6 py-4">
                                    <div className="font-bold text-slate-800 text-sm">{u.username}</div>
                                    <div className="text-[10px] text-gray-400">Contraseña oculta por seguridad</div>
                                </td>
                                <td className="px-6 py-4">
                                    <span className="text-sm text-slate-700">{u.sector?.nombre || 'Sin Sector'}</span>
                                </td>
